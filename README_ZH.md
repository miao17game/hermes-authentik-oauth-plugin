...
- **Access Token**（短期）和 **Refresh Token**（长期）存储在 `HttpOnly` cookie 中。
- Access Token 过期后，中间件会使用 Refresh Token 透明续期。
- `PKCE S256` 用于降低截获风险；JWT 签名通过 Authentik 的 `JWKS` 端点验证。
...
| `PyJWT[crypto]` | JWT 签名验证（RS256/ES256 等）和 JWKS 客户端支持 |
